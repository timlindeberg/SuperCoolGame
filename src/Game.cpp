...
void Game::PrintIntroText() const{
	Lab3::out << BeginBox(Format::CYAN);
	Lab3::out << Alignment::CENTER << "Welcome to Super cool game! " << std::endl;
	Lab3::out << Delimiter();
	Lab3::out << "You are a space merchant on an exploration mission in deep space. ";
	Lab3::out << "While exploring an uncharted quadrant in search of impossibletogettium aliens attacked! ";
	Lab3::out << "They're unlike anything you have ever seen before and have already breached your shield defences. ";
	Lab3::out << "Your only chance is to get to an escape pod before you are captured! " << std::endl;
	Lab3::out << Delimiter();
...
Actor* Game::GetNextActor(const std::vector<std::string>& executedMove) const {
	...
	for(auto& a : GetPlayer()->Location()->Actors()){
			Actor* actor = a.get(); 
			actors.push_back(actor);
		}
