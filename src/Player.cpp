...
void Player::InitCommandMap(){
	...
	P_ENTRY("talk", 	 TalkCommand, 	NORMAL, "Talk to a friendly character." 					  );
...
bool Player::TakeCommand(const std::vector<std::string>& command){
	...
	PRINT_BOX(GameStream::SUCCESS_COLOR,
		"You took " << COLOR(item->Name(), GREEN) << ". " << item->Description() << std::endl);
...
bool Player::TalkCommand(const std::vector<std::string>& command){
	if(command.size() == 0){
		PRINT_BOX(GameStream::FAIL_COLOR,
			"Who do you want to talk to?" << std::endl);
		return false;
	}
	...
	if(dynamic_cast<Player*>(actor)){
		...
		Lab3::out << "You have an amusing conversation with yourself." << std::endl;
		for(auto& actor : _location->Actors()){
			FriendlyActor* friendly = dynamic_cast<FriendlyActor*>(actor.get());
			if(friendly){
				Lab3::out << COLOR(friendly->Name(), BLUE) << " thinks you're weird." << std::endl;
				break;
			}
		}
		Lab3::out << EndBox();
	...
	FriendlyActor* friendlyActor = dynamic_cast<FriendlyActor*>(actor);
	if(!friendlyActor){
		PRINT_BOX(GameStream::FAIL_COLOR,
			"You can not talk to " << COLOR(actorName, RED) << "!" << std::endl);
		return false;
	}
