...
public:
	...
	static std::string& FirstUpperCase(std::string& s);
	...
	template<class T>
	static bool SameName(const T& t1, const T& t2);

	template<class T>
	static bool SameName(const T& t, const std::string& name);

	static bool SameName(const std::string& t, const std::string& name);
	...
	template<class T>
	static void RemoveNullObjects(std::vector<T*> vec);

	template<class T, class Func>
	static void SafeForEach(std::vector<T*> vec, Func f);
	...
	static float NormalDistFloat(double mean, double stdDev);
