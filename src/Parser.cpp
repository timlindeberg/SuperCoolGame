...
const std::string Parser::PROMPT = u8"▶";
...
std::vector<std::string> Parser::ParseCommand() {
	...
	while(words.size() == 0){
		...
		// Move input cursor
		Lab3::out << MoveCursor(-2, GameStream::OUTER_INDENT_WIDTH + GameStream::INDENT_WIDTH);
		std::cout << COLOR(PROMPT, MAGENTA);
		Lab3::out << MoveCursor(0, 1);
...
bool Parser::GetPreviousCommand(const std::string& command, std::vector<std::string>& outCommand){
	try{
		...
		if(i >= 1 && i <= _count){
			int index = EuclidianMod(_index - i, NUM_PREVIOUS_COMMANDS);
...
void Parser::PrintPreviousCommands(){
	for(size_t i = 0; i < _count; ++i){
		int index = EuclidianMod(_index - i - 1, NUM_PREVIOUS_COMMANDS);
...
int Parser::EuclidianMod(int a, int b) const{
	int r = a % b;
