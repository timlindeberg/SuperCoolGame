...
public:
	...
	IO();
	IO(const std::string& name, const std::string& description);
	virtual ~IO();
	...
	template<class T>
	static T* ParseObjectRaw(std::istream& is);
	...
	template<class T>
	static std::vector<T*> ParseListRaw(std::istream& is);
	template<class T>
	static std::vector<std::unique_ptr<T>> ParseList(std::istream& is);

	static std::vector<std::string> ParseStringList(std::istream& is);

	template<class T>
	static void PrintList(std::ostream& os, const std::vector<T>& list);

	static void PrintStringList(std::ostream& os, const std::vector<std::string>& list);
